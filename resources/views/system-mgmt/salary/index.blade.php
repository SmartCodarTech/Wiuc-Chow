...
@section('action-content')
  ...
  <div class="box-body">
    ...
    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            <thead>
              <tr role="row">
                ...
                <th width="10%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="country: activate to sort column ascending">Premium</th>
            ...
            @foreach ($premiums as $premium)
                <tr role="row" class="odd">
                  <td><img src="../{{$premium->employees_picture }}" width="50px" height="50px"/></td>
                  <td class="sorting_1">{{ $premium->employees_firstname }}  {{$premium->employees_lastname}}</td>
                  <td>{{ $premium->premium_type }}</td>
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('premium.destroy', ['id' => $premium->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        ...
                        <a href="{{ route('premium.edit', ['id' => $premium->id]) }}" class="btn btn-info col-sm-3 col-xs-5 btn-margin">
                        Update
                        </a>
            ...
            <tfoot>
              <tr>
                <th width="20%" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Picture: activate to sort column descending" aria-sort="ascending">Picture</th>
                <th width="12%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="country: activate to sort column ascending">Full Name</th>
      ...
      <div class="row">
        <div class="col-sm-5">
          <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">Showing 1 to {{count($premiums)}} of {{count($premiums)}} entries</div>
        </div>
        <div class="col-sm-7">
